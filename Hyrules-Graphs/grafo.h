...
class Grafo {
    ...
    public:
       ...
       QString transbordosMin(Ciudad* pOrigen, Ciudad* pDestino) {
            vector<Ciudad*> aux;
        ...
        QString manaMin(int pOrigen, int pDestino){
            int valorRuta = dijkstra(pOrigen, &matrizMana, pDestino);
            QString aux = QString::number(valorRuta);
            QString txt = "Menor ruta en mana desde" + listaNodos.at(pOrigen)->getDato()->getNombre() + " hacia " + listaNodos.at(pDestino)->getDato()->getNombre() + " tiene un coste de: " + aux + " mana";
            return txt;
        }

        //recorrido duración mínima
        QString duracionMin(int pOrigen, int pDestino){
            int valorRuta = dijkstra(pOrigen, &matrizDuracion, pDestino);
            QString aux = QString::number(valorRuta);
            QString txt = "Menor ruta en tiempo desde" + listaNodos.at(pOrigen)->getDato()->getNombre() + " hacia " + listaNodos.at(pDestino)->getDato()->getNombre() + " tiene una duracion de: " + aux + " minutos";
            return txt;
        }

        //https://www.geeksforgeeks.org/c-program-for-dijkstras-shortest-path-algorithm-greedy-algo-7/
        int dijkstra(int pOrigen, vector<vector<int>>* matrizGrafo, int pDestino){//número de como se metió al grafo
        ...
        int getIntbyCiudad(Ciudad* pCiudad){

            int indice;

            for (int i=0; i<listaNodos.size(); i++){
                if(listaNodos.at(i)->getDato()->getId()==pCiudad->getId()){
                    indice=i;
                }
            }
